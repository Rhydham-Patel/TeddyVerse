...
const Conversation = ({ conversation }: { conversation: any }) => {
	...
	const activeBgclassName = selectedConversation?._id === conversation._id;
	return (
		<>
			<div className={`flex gap-2 items-center p-3 hover:bg-chat-hover cursor-pointer m-2 rounded ${activeBgclassName ? 'bg-gray-tertiary' : ''} `} onClick={() => setSelectedConversation(conversation)}>
				<Avatar className='border border-gray-900 bg-[#ffff] overflow-visible relative'>
			...
			<hr className='h-[1px] mx-10 bg-gray-secondary' />
		</>
	);
};
