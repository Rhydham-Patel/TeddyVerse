...
const ChatBubble = ({message, me, previousMessage, onVideoLoad}: ChatBubbleProps) => {
	...
	const bgclassName = fromMe ? "bg-green-chat" : "bg-white dark:bg-gray-primary";
	...
	const renderMessageContent = () => {
		switch (message.messageType) {
			...
			case "video":
				return <VideoMessage message={message} onVideoLoad={onVideoLoad} />;
			default:
				return null;
		}
	}
	...
	if (!fromMe) {
		...
		<ChatBubbleAvatar message={message} isMember={isMember} isGroup={isGroup}/>
			<div className={`flex flex-col z-20 max-w-fit px-2 pt-1 rounded-md shadow-md relative ${bgclassName}`}>
	...
	return (
		...
		<div className='flex gap-1 w-2/3 ml-auto'>
				<div className={`flex  z-20 max-w-fit px-2 pt-1 rounded-md shadow-md ml-auto relative ${bgclassName}`}>
...
const VideoMessage = ({ message, onVideoLoad }: { message: IMessage; onVideoLoad: () => void }) => {
	return (
		<div className="w-[250px] m-2 relative">
			<ReactPlayer
				url={message.content}
				width={250}
				height="auto"
	...
	return (
		<div>
			{isLink ? (
				...
				<p className={`mr-2 text-sm text-black dark:text-white dark:font-light`}>{message.content}</p>
			)}
		</div>
	);
};
