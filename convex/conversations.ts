...
export const getMyConversations = query({
    ...
    handler: async (ctx, args) => {
        ...
        const conversationsWithDetails = await Promise.all(myconversations.map(async (conversation) => {
            ...
            .filter(q => q.eq(q.field("conversation"), conversation._id)).order('desc').take(1);

            return {...userDetails, ...conversation, lastMessage: lastMessage[0] || null};
        }))
